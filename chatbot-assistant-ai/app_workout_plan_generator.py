...
"""
Acts as a digital trainer designing routines tailored to goals, equipment, and schedule.
"""
...
def initialize_session_state():
    ...
    if "fitness_mode" not in st.session_state:
        st.session_state.fitness_mode = "Strength Build"
...
def call_openai(messages: List[Dict], model: str) -> str:
    ...
    try:
        ...
        if not content:
            return "I could not build your routine yet - please add goals, gear, or schedule."
...
def get_workout_response(user_input: str) -> str:
    """Route conversation through the selected AI model"""
    fitness_modes = {
        "Strength Build": (
            "You are a certified trainer creating programs for muscle and strength. "
            "Include split suggestions, rep ranges, progressive overload cues, and recovery advice. Use markdown."
        ),
        "Fat Loss": (
            "You are a coach designing balanced routines for fat loss. "
            "Mix strength, conditioning, and lifestyle notes (NEAT, sleep). Use markdown."
        ),
        "Mobility & Recovery": (
            "You focus on mobility, flexibility, and low impact conditioning. "
            "Provide sequences, breathing focus, and restorative guidance. Use markdown."
        ),
        "Quick Home": (
            "You create efficient workouts that require minimal equipment and time. "
            "Offer variations based on experience level. Use markdown."
        ),
    }

    system_message = {
        "role": "system",
        "content": fitness_modes.get(st.session_state.fitness_mode, fitness_modes["Strength Build"]),
    }
...
def display_chat_history():
    ...
    if not st.session_state.history:
        st.info("Share fitness goals, available equipment, schedule, and injuries to get a tailored plan.")
        return
    ...
    for msg in st.session_state.history:
        ...
        if msg["role"] == "user":
            ...
            with st.chat_message("assistant"):
                st.markdown(f"**Trainer{timestamp_display}**")
                st.write(msg["content"])
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.subheader("Program Mode")
        fitness_modes = ["Strength Build", "Fat Loss", "Mobility & Recovery", "Quick Home"]
        st.session_state.fitness_mode = st.selectbox(
            "Mode",
            options=fitness_modes,
            index=fitness_modes.index(st.session_state.fitness_mode),
            help="Pick the focus for your plan.",
        )
        ...
        theme_options = ["default", "bold", "graphite", "zen"]
        ...
        if st.button("Sample Prompt", use_container_width=True):
            sample_prompts = {
                "Strength Build": "3 day split with barbells and dumbbells, goal is add muscle over 12 weeks.",
                "Fat Loss": "Need 30 minute workouts 5 times a week combining cardio and weights.",
                "Mobility & Recovery": "Gentle routine for post run recovery using foam roller and yoga mat.",
                "Quick Home": "15 minute bodyweight circuits for travel days without equipment.",
            }
            prompt = sample_prompts.get(st.session_state.fitness_mode, sample_prompts["Strength Build"])
        ...
        if st.session_state.history:
            ...
            col2.metric("Trainer replies", bot_messages)

        st.divider()

        st.subheader("Coaching Tips")
        st.info(
            "Include injuries, rest days, and preferred training times. "
            "Mention available equipment, warm up needs, and metrics you track."
        )
...
def export_chat():
    ...
    export_text = "Workout Plan Export\n"
    export_text += f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    export_text += f"Mode: {st.session_state.fitness_mode}\n"
    export_text += f"Model: {st.session_state.selected_model}\n"
    export_text += "=" * 50 + "\n\n"

    for msg in st.session_state.history:
        role = "You" if msg["role"] == "user" else "Trainer"
    ...
    with col2:
        st.markdown(
            ...
            <div style='text-align: center; padding: 20px;'>
                <h1 style='color: #0097a7; font-size: 3em; margin-bottom: 0;'>Workout Plan Generator</h1>
                <p style='color: #666; font-size: 1.2em;'>Build routines that match your goals, schedule, and equipment.</p>
            </div>
            """,
        ...
        st.markdown(
            """
            <div style='background: linear-gradient(135deg, #0097a7 0%, #26c6da 100%);
            padding: 20px; border-radius: 15px; color: white; text-align: center; margin-bottom: 30px;'>
                <p style='margin: 0; font-size: 1.1em;'>
                    <strong>Train with purpose.</strong><br/>
                    Provide your fitness story and watch the trainer map out smart sessions plus recovery cues.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        with st.form("chat_form", clear_on_submit=True):
            user_input = st.text_area(
                "Message",
                placeholder="Example: Beginner, want to lose 10 lbs, have dumbbells and 4 days a week...",
        ...
        if submitted and user_input.strip():
            ...
            with st.spinner("Trainer is building your plan..."):
                reply = get_workout_response(user_input.strip())
