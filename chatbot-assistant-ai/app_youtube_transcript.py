...
"""
This tool summarizes transcripts from YouTube videos—great for turning long educational
content, podcasts, or lectures into quick takeaways. Users can paste transcripts directly
(from auto-generated captions or tools like YouTube Transcript) and get a concise summary
"""
...
def get_summary_response(user_input: str) -> str:
    """
    Get YouTube transcript summary from selected AI model
    """
    summary_styles = {
        "Concise": (
            "You are an expert summarizer. Your task is to take long video transcripts "
            "and summarize them into key insights and bullet points. Be concise, clear, and highlight "
            "the most important information. Focus on main ideas, key takeaways, and actionable insights. "
        ...
        "Detailed": (
            "You are an expert summarizer. Your task is to take long video transcripts "
            "and provide a comprehensive summary with detailed explanations. Include context, main arguments, "
            "supporting points, examples mentioned, and key conclusions. Structure the summary with clear sections. "
            "Format your responses using plain text with markdown. Never use HTML tags like <br>, <b>, <i>, etc. "
            "Use markdown syntax instead (line breaks, **bold**, *italic*)."
        ),
        "Key Points": (
            "You are an expert summarizer. Extract the most critical information from video transcripts "
            "and present them as numbered key points. Each point should be self-contained and actionable. "
            "Focus on facts, statistics, recommendations, and important statements. "
            "Format your responses using plain text with markdown. Never use HTML tags like <br>, <b>, <i>, etc. "
            "Use markdown syntax instead (line breaks, **bold**, *italic*)."
        ),
        "Q&A Style": (
            "You are an expert summarizer. Transform video transcripts into a Q&A format. "
            "Extract key questions that were addressed and provide clear, concise answers based on the content. "
            "This format helps with quick reference and study. "
            "Format your responses using plain text with markdown. Never use HTML tags like <br>, <b>, <i>, etc. "
            "Use markdown syntax instead (line breaks, **bold**, *italic*)."
        ),
    }
    ...
    system_message = {
        "role": "system",
        "content": summary_styles.get(st.session_state.summary_style, summary_styles["Concise"]),
    }
...
def display_chat_history():
    ...
    if not st.session_state.history:
        st.info("🎥 Paste a YouTube transcript and get an instant summary! Perfect for lectures, podcasts, or long videos.")
        return
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        summary_styles = ["Concise", "Detailed", "Key Points", "Q&A Style"]
        st.session_state.summary_style = st.selectbox(
            ...
            help="Choose how you want the transcript summarized",
        )
        ...
        theme_options = ["default", "dark", "youtube", "minimal"]
        ...
        if st.button("Sample Transcript", use_container_width=True):
            sample_transcript = (
                "In today's lecture, we'll cover the fundamentals of machine learning. "
                "Machine learning is a subset of artificial intelligence that focuses on "
                "building systems that can learn from data. There are three main types: "
                "supervised learning, where we have labeled data; unsupervised learning, "
                "where we work with unlabeled data; and reinforcement learning, where "
                "an agent learns through trial and error. Let's dive deeper into each..."
            )
            ...
            with st.spinner("Generating summary..."):
                reply = get_summary_response(sample_transcript)
            st.session_state.history.append({
                "role": "assistant",
                "content": reply,
                "timestamp": datetime.now().isoformat(),
        ...
        if st.session_state.history:
            ...
            col1, col2 = st.columns(2)
            col1.metric("Transcripts", user_messages)
        ...
        st.info(
            ...
            "- Try browser extensions like 'YouTube Transcript'\n"
            "- Copy and paste the entire transcript for best results"
        )
...
def export_chat():
    ...
    export_text = "YouTube Transcript Summarizer Export\n"
    export_text += f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    export_text += f"Style: {st.session_state.summary_style}\n"
    export_text += f"Model: {st.session_state.selected_model}\n"
    export_text += "=" * 50 + "\n\n"

    for msg in st.session_state.history:
        role = "Transcript" if msg["role"] == "user" else "Summary"
    ...
    with col2:
        st.markdown(
            ...
            <div style='text-align: center; padding: 20px;'>
                <h1 style='color: #d32f2f; font-size: 3em; margin-bottom: 0;'>🎥 YouTube Transcript Summarizer</h1>
                <p style='color: #666; font-size: 1.2em;'>Turn long videos into quick, actionable summaries</p>
            </div>
            """,
        ...
        st.markdown(
            ...
            padding: 20px; border-radius: 15px; color: white; text-align: center; margin-bottom: 30px;'>
                <p style='margin: 0; font-size: 1.1em;'>
                    <strong>Save time on long videos!</strong><br/>
                    Paste any YouTube transcript and get instant summaries—perfect for lectures, podcasts, and tutorials.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
