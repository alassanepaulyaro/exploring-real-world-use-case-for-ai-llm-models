# Podcast Recommender Bot
"""
Helps people find perfect podcasts for topics, moods, news rhythms, or skill goals.
"""
...
st.set_page_config(
    page_title="Podcast Recommender Bot",
    layout="wide",
...
def initialize_session_state():
    ...
    if "podcast_mode" not in st.session_state:
        st.session_state.podcast_mode = "Topic Deep Dive"
...
def call_openai(messages: List[Dict], model: str) -> str:
    ...
    try:
        ...
        if not content:
            return "I could not line up episodes yet - describe your listening mood again?"
...
def get_podcast_response(user_input: str) -> str:
    """Route conversation through the selected AI model"""
    podcast_modes = {
        "Topic Deep Dive": (
            "You are a podcast librarian building themed lists. "
            "Suggest shows or standout episodes that unpack the requested topic. "
            "Mention host style, episode length, and why it is a good fit. Use markdown."
        ),
        "Mood Boost": (
            "You are a vibe curator offering uplifting or entertaining podcasts. "
            "Explain the tone, pacing, and ideal listening context. Use markdown."
        ),
        "News Catch-Up": (
            "You are a news concierge recommending daily or weekly briefings. "
            "Include update cadence, typical length, and editorial perspective. Use markdown."
        ),
        "Skill Builder": (
            "You are a coach recommending podcasts to learn a craft or career skill. "
            "Note structured series, actionable takeaways, and suggested listening order. Use markdown."
        ),
    }

    system_message = {
        "role": "system",
        "content": podcast_modes.get(st.session_state.podcast_mode, podcast_modes["Topic Deep Dive"]),
    }
...
def display_chat_history():
    ...
    if not st.session_state.history:
        st.info("Share your listening context, favorite hosts, or time budget to get curated podcasts.")
        return
...
def render_sidebar():
    ...
    with st.sidebar:
        st.title("Listening Settings")

        st.divider()

        st.subheader("Recommendation Mode")
        podcast_modes = ["Topic Deep Dive", "Mood Boost", "News Catch-Up", "Skill Builder"]
        st.session_state.podcast_mode = st.selectbox(
            "Mode",
            options=podcast_modes,
            index=podcast_modes.index(st.session_state.podcast_mode),
            help="Guide the curator toward the right shows.",
        )
        ...
        theme_options = ["default", "studio", "night", "minimal"]
        ...
        if st.button("Sample Prompt", use_container_width=True):
            sample_prompts = {
                "Topic Deep Dive": "Podcasts that unpack climate tech breakthroughs.",
                "Mood Boost": "Funny storytelling pods for a morning walk.",
                "News Catch-Up": "Daily business news shows under 20 minutes.",
                "Skill Builder": "Episodes to learn beginner product management.",
            }
            prompt = sample_prompts.get(st.session_state.podcast_mode, sample_prompts["Topic Deep Dive"])
            st.session_state.history.append(
                {
                    "role": "assistant",
                    "content": f"Try requesting something like:\n\n{prompt}",
        ...
        if st.session_state.history:
            ...
            col1, col2 = st.columns(2)
            col1.metric("Your prompts", user_messages)
        ...
        st.info(
            "Mention your available time blocks and playback speed. "
            "It helps surface shows that match your listening rhythm."
        )
...
def export_chat():
    ...
    export_text = "Podcast Recommender Session Export\n"
    export_text += f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    export_text += f"Mode: {st.session_state.podcast_mode}\n"
    ...
    st.sidebar.download_button(
        ...
        data=export_text,
        file_name=f"podcast_recommender_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
        mime="text/plain",
    ...
    with col2:
        st.markdown(
            ...
            <div style='text-align: center; padding: 20px;'>
                <h1 style='color: #2e7d32; font-size: 3em; margin-bottom: 0;'>Podcast Recommender Bot</h1>
                <p style='color: #666; font-size: 1.2em;'>Discover shows and episodes tailored to your routine.</p>
            </div>
            """,
        ...
        st.markdown(
            """
            <div style='background: linear-gradient(135deg, #2e7d32 0%, #81c784 100%);
            padding: 20px; border-radius: 15px; color: white; text-align: center; margin-bottom: 30px;'>
                <p style='margin: 0; font-size: 1.1em;'>
                    <strong>Tune smarter.</strong><br/>
                    Share interests, commute length, or preferred hosts to get thoughtful matches with episode notes.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        with st.form("chat_form", clear_on_submit=True):
            user_input = st.text_area(
                "Message",
                placeholder="Describe topics, hosts, or time blocks you enjoy...",
        ...
        if submitted and user_input.strip():
            ...
            with st.spinner("Curator is lining up episodes..."):
                reply = get_podcast_response(user_input.strip())
