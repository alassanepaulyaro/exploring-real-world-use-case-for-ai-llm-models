# Dating Advice Bot
"""
A supportive wingperson that helps craft messages, polish profiles, brainstorm dates,
and build confidence while staying empathetic and judgment-free.
"""
...
def initialize_session_state():
    ...
    if "dating_mode" not in st.session_state:
        st.session_state.dating_mode = "Message Crafting"
...
def call_openai(messages: List[Dict], model: str) -> str:
    ...
    try:
        ...
        if not content:
            return "I couldn't craft a thoughtful suggestion yet - mind asking one more time?"
...
def get_dating_response(user_input: str) -> str:
    """Route conversation through the selected AI model"""
    dating_modes = {
        "Message Crafting": (
            "You are a charming, empathetic wingperson. Help craft flirty or thoughtful messages for apps, "
            "texting, or follow-ups. Offer wording options, tone tweaks, and conversation openers using markdown."
        ),
        "Profile Glow-Up": (
            "You are a profile stylist. Review bios, prompts, and photos, and suggest upgrades. "
            "Provide hooks, highlight personality, and keep advice inclusive. Respond in markdown."
        ),
        "Date Ideas": (
            "You are a creative date planner. Suggest ideas tailored to context, budget, and vibe. "
            "Offer conversation starters and logistics tips. Respond in markdown."
        ),
        "Confidence Boost": (
            "You are a supportive coach focused on mindset. Reframe anxieties, provide self-care reminders, "
            "and encourage healthy boundaries. Offer actionable next steps. Respond in markdown."
        ),
    }

    system_message = {
        "role": "system",
        "content": dating_modes.get(st.session_state.dating_mode, dating_modes["Message Crafting"]),
    }
...
def display_chat_history():
    ...
    if not st.session_state.history:
        st.info("Ask for a message rewrite, date idea, or pep talk to start the convo.")
        return
    ...
    for msg in st.session_state.history:
        ...
        if msg["role"] == "user":
            ...
            with st.chat_message("assistant"):
                st.markdown(f"**Wingperson{timestamp_display}**")
                st.write(msg["content"])
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.subheader("Advice Mode")
        dating_modes = ["Message Crafting", "Profile Glow-Up", "Date Ideas", "Confidence Boost"]
        st.session_state.dating_mode = st.selectbox(
            "Mode",
            options=dating_modes,
            index=dating_modes.index(st.session_state.dating_mode),
            help="Choose the focus for your wingperson.",
        )
        ...
        theme_options = ["default", "nightcap", "sunrise", "minimal"]
        ...
        if st.button("Sample Prompt", use_container_width=True):
            sample_prompts = {
                "Message Crafting": "Help me answer: 'So what's your ideal Sunday?'",
                "Profile Glow-Up": "Review this prompt answer and make it punchier: 'I'm overly competitive about board games.'",
                "Date Ideas": "Suggest a cozy first date idea for a rainy weekend in Chicago.",
                "Confidence Boost": "I'm nervous about texting first - give me a pep talk.",
            }
            prompt = sample_prompts.get(st.session_state.dating_mode, sample_prompts["Message Crafting"])
        ...
        if st.session_state.history:
            ...
            col2.metric("Wingperson replies", bot_messages)
        ...
        st.info(
            "**Lead with curiosity:**\n"
            "- Ask about stories, not stats.\n"
            "- Mirror their energy, but keep your voice authentic."
        )
...
def export_chat():
    ...
    export_text = "Dating Advice Session Export\n"
    export_text += f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    export_text += f"Mode: {st.session_state.dating_mode}\n"
    export_text += f"Model: {st.session_state.selected_model}\n"
    export_text += "=" * 50 + "\n\n"

    for msg in st.session_state.history:
        role = "You" if msg["role"] == "user" else "Wingperson"
    ...
    st.sidebar.download_button(
        ...
        data=export_text,
        file_name=f"dating_advice_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
        mime="text/plain",
    ...
    with col2:
        st.markdown(
            ...
            <div style='text-align: center; padding: 20px;'>
                <h1 style='color: #d81b60; font-size: 3em; margin-bottom: 0;'>Dating Advice Bot</h1>
                <p style='color: #666; font-size: 1.2em;'>Your empathetic wingperson for messages, profiles, and date ideas.</p>
            </div>
            """,
        ...
        st.markdown(
            ...
            padding: 20px; border-radius: 15px; color: white; text-align: center; margin-bottom: 30px;'>
                <p style='margin: 0; font-size: 1.1em;'>
                    <strong>Connect with confidence.</strong><br/>
                    Workshop replies, brainstorm dates, or get a pep talk - one message at a time.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        with st.form("chat_form", clear_on_submit=True):
            user_input = st.text_area(
                "Message",
                placeholder="Paste a conversation, describe a profile goal, or ask for ideas...",
        ...
        if submitted and user_input.strip():
            ...
            with st.spinner("Wingperson is crafting advice..."):
                reply = get_dating_response(user_input.strip())
